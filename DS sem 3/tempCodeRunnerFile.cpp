#include <bits/stdc++.h>
#include <conio.h> // for clrsc(),getch()
using namespace std;
...
class TrainSchedule {//GARV MADAN
public:
    struct TreeNode {
        Train train;
        TreeNode* left;
    ...
    void printAllTrains(TreeNode* node) {//INORDER TRAVERSAL IS DUSED HERE
...
void waitlistshift(int train_id, int canceledSeat) {
    ...
    ofstream tempFileStream(tempFile);  // Temporary file to store updated data
    
    string passengerfirstName, passengerlastName, gender;
    int age, trainId, seat;

    // Process each passenger record
    while (file1 >> passengerfirstName >> passengerlastName >> age >> gender >> trainId >> seat) {
        // Check if the passenger is on the same train and has a seat number higher than the canceled seat
        if (trainId == train_id && seat > canceledSeat) {
            seat -= 1;  // Shift seat number down by 1 for passengers with seat numbers above the canceled seat
        }
        
        // Write updated passenger info to the temp file
        tempFileStream << passengerfirstName << " " << passengerlastName << " " << age << " " 
                       << gender << " " << trainId << " " << seat << "\n";
    }
